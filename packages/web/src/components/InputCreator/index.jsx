...
function InputCreator(props) {
  ...
  if (dependsOn && showWhen && watch) {
    const dependentFieldName = Array.isArray(dependsOn) ? dependsOn[0] : dependsOn;
    const dependentValue = watch(dependentFieldName);
    if (dependentValue !== showWhen) {
      return null; // Hide field if condition not met
    }
  }
  
  // Make field required if it's visible and depends on a condition
  const isConditionallyRequired = dependsOn && showWhen && watch && required;
  const effectivelyRequired = isConditionallyRequired ? watch(Array.isArray(dependsOn) ? dependsOn[0] : dependsOn) === showWhen : required;
